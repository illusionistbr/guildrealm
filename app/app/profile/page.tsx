...
import {
  User, Camera, Save, Image as ImageIcon,
  Eye, EyeOff, Link as LinkIcon, Trash2, MessageCircle, Lock,
} from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const [bio, setBio] = useState('Aventureiro apaixonado por explorar masmorras e colecionar conquistas. Líder dos Dragões de Fogo.');
  ...
  const handleFileSelect = (
    ...
    if (!allowed.includes(file.type)) {
      alert('Formato não aceito. Use JPEG, PNG ou GIF.');
      return;
    }
    ...
    <motion.div initial="initial" animate="animate" variants={{ animate: { transition: { staggerChildren: 0.05 } } }} className="space-y-6">
      <motion.div variants={fadeUp} className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted mt-1">Gerencie sua aparência, links sociais e visibilidade.</p>
        </div>
        <button onClick={handleSave} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-accent text-white text-sm font-medium hover:bg-accent-hover transition-colors">
          {saved ? <><Save size={16} /> Salvo!</> : <><Save size={16} /> Salvar</>}
        </button>
      </motion.div>

      {/* Cover + Avatar */}
      <motion.div variants={fadeUp} className="rounded-xl border border-[rgba(38,51,86,0.5)] bg-gradient-to-br from-[rgba(19,29,48,0.6)] to-[rgba(10,18,32,0.4)] overflow-hidden">
        {/* Cover */}
        <div className="relative h-48 md:h-56 bg-[#0a1122]">
          ...
          <div className="absolute bottom-3 right-3 flex gap-2">
            <input ref={coverInput} type="file" accept="image/jpeg,image/png,image/gif" className="hidden" onChange={(e) => handleFileSelect(e, 'cover')} />
            <button
              onClick={() => coverInput.current?.click()}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[rgba(5,9,18,0.7)] border border-[rgba(38,51,86,0.5)] text-white text-xs backdrop-blur-md hover:border-accent/30 transition-colors"
            >
              <Camera size={14} /> {cover ? 'Trocar capa' : 'Adicionar capa'}
            </button>
            {cover && (
              <button
                onClick={() => setCover(null)}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-500/20 border border-red-500/30 text-red-400 text-xs backdrop-blur-md hover:bg-red-500/30 transition-colors"
              >
                <Trash2 size={14} />
              </button>
            )}
        ...
        <div className="px-6 pb-6 -mt-12 relative z-10">
          <div className="flex items-end gap-5">
            <div className="relative">
              ...
              <input ref={avatarInput} type="file" accept="image/jpeg,image/png,image/gif" className="hidden" onChange={(e) => handleFileSelect(e, 'avatar')} />
              <button
                onClick={() => avatarInput.current?.click()}
                className="absolute bottom-0 right-0 w-8 h-8 rounded-full bg-accent text-white flex items-center justify-center hover:bg-accent-hover transition-colors border-2 border-[#050912]"
              >
                <Camera size={14} />
              </button>
            </div>
            <div className="pb-1">
              <p className="text-white font-heading font-bold text-lg">{displayName}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
            <div>
              <label className="block text-sm text-muted mb-1.5">Nome de exibição</label>
              <input type="text" value={displayName} onChange={(e) => setDisplayName(e.target.value)} className="w-full h-10 px-3 bg-[#0a1122] border border-[rgba(38,51,86,0.5)] rounded-lg text-sm text-white focus:outline-none focus:border-accent/50 transition-colors" />
            </div>
            <div>
              ...
              <div className="flex items-center h-10">
                <span className="text-muted text-sm bg-[#0a1122] border border-r-0 border-[rgba(38,51,86,0.5)] rounded-l-lg px-3 h-full flex items-center whitespace-nowrap">/profile/</span>
                <input
                  type="text"
                  value={nickname}
                  readOnly
          ...
          <div className="mt-4">
            ...
            <textarea
              ...
              className="w-full px-3 py-2 bg-[#0a1122] border border-[rgba(38,51,86,0.5)] rounded-lg text-sm text-white focus:outline-none focus:border-accent/50 transition-colors resize-none"
            />
      ...
      <motion.div variants={fadeUp} className="rounded-xl border border-[rgba(38,51,86,0.5)] bg-gradient-to-br from-[rgba(19,29,48,0.6)] to-[rgba(10,18,32,0.4)] p-6">
        ...
        <p className="text-sm text-muted mb-4">Escolha o que outros jogadores podem ver no seu perfil público.</p>
        <div className="space-y-3">
          {([
            ...
            <div key={opt.key} className="flex items-center justify-between py-2 border-b border-[rgba(38,51,86,0.3)] last:border-0">

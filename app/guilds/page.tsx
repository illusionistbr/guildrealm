...
export default function GuildsPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snap) => {
        const list: GuildDoc[] = [];
        snap.forEach((doc) =>
          list.push({ id: doc.id, ...doc.data() } as GuildDoc),
        );
        list.sort(
  ...
  useEffect(() => {
    ...
    const needed = [
      ...new Set(
        guilds
          .filter((g) => !g.ownerName && g.ownerId)
          .map((g) => g.ownerId as string),
      ),
  ...
  const filtered = useMemo(() => {
    ...
    return guilds.filter((g) => {
      ...
      if (
        filters.recruitment !== 'all' &&
        g.recruitment !== filters.recruitment
      )
        return false;
      return true;
    });
  ...
  const gameLabel = (value?: string) =>
    games.find((g) => g.value === value)?.label ?? '—';
        ...
        <div className="filter-panel">
          ...
          <div className="filter-row">
            ...
            <FilterSelect
              ...
              options={[
                { value: 'all', label: t('filterAnyRegion') },
                ...regions,
              ]}
            />
            <FilterSelect
              ...
              onChange={(value) =>
                setFilters((prev) => ({ ...prev, recruitment: value }))
              }
        ...
        {loading ? (
          <div className="guild-grid">
            {[0, 1, 2].map((i) => (
              <div
                key={i}
                className="guild-card h-72 animate-pulse"
                style={{ opacity: 0.6 }}
              />
            ))}
          </div>
                ...
                >
                  ...
                  <div className="guild-body">
                    ...
                    <p className="guild-members">
                      <UsersRound size={15} />{' '}
                      {t('members', { count: guild.members?.length ?? 0 })}
                    </p>
